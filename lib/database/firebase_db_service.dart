...
class FirebaseDBService {
  ...
  Future <void> delete ({required String path}) async {
    ...
    try {
      ...
      await delete(path: "todos/$todoId");
      print("ToDo with ID $todoId deleted from Firebase.");

      // Delete from local SQLite database
      await DatabaseHelper.instance.deleteToDo(todoId);
      print("ToDo with ID $todoId deleted from local database.");
    } catch (e) {
      print("Error deleting ToDo with ID $todoId: $e");
      // You might want to rethrow the error or handle it more gracefully
...
// Sync to firebase
  ...
  //Sync from firebase
  Future<void> syncFromFirebase(String currentUserId) async {
  final dbHelper = DatabaseHelper.instance;
